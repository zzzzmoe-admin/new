...
class BLEFusionExploitApp(App):
    def build(self):
        self.title = "TTLock + 中国情趣玩具 融合攻击平台 - Grok 4 Developer Mode V11 (低强度版)"
        ...
        slider_box.add_widget(Label(text='情趣玩具实时强度 (0-100):', size_hint_x=0.32, font_size=18))
        ...
        self.status = Label(text='Grok 4 Developer Mode V11 | Root / 无 Root 双模式 (情趣玩具低强度)',
    ...
    async def send_vibrate(self, intensity):
        if self.current_write_char and self.current_client and self.is_sextoy:
            intensity = min(100, max(0, int(intensity)))  # 严格限制在 0-100
